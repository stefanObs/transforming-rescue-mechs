...
- **Ground tiles for repeating maps:** flat cel iso diamonds or continuous fill — **no** per-tile black outlines (those form a grid). Thick `#1A1A1A` outlines belong on characters/props only. Perspective “hero” building/terrain renders are props, never tiled as grass/road fills.
