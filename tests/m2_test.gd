...
func _run() -> void:
	...
	if packed is PackedScene:
		...
		_assert(is_equal_approx(float(player.get("SPEED_ROBOT")), 350.0), "SPEED_ROBOT == 350")
		_assert(is_equal_approx(float(player.get("SPEED_VEHICLE")), 650.0), "SPEED_VEHICLE == 650")
		_assert(is_equal_approx(float(player.get("SPEED_VEHICLE_RUSH")), 900.0), "SPEED_VEHICLE_RUSH == 900")
		_assert(is_equal_approx(float(player.get("WALK_FPS")), 14.0), "WALK_FPS == 14")
		player.call("set_character", "bolt")
		var bolt_v := float(player.call("_vehicle_speed"))
		_assert(is_equal_approx(bolt_v, 650.0), "bolt vehicle speed == 650")
		player.call("set_character", "marina")
		_assert(player.get_node("RobotSprite").texture != null, "marina robot texture")
		var marina_v := float(player.call("_vehicle_speed"))
		_assert(is_equal_approx(marina_v, 650.0), "marina vehicle speed == 650")
		...
		var rush_v := float(player.call("_vehicle_speed"))
		_assert(is_equal_approx(rush_v, 900.0), "rush vehicle speed == 900")
		_assert(rush_v > bolt_v, "rush vehicle speed > bolt")
		_assert(rush_v > marina_v, "rush vehicle speed > marina")
